 ...
 */
public class ServletPanelUsuario extends HttpServlet {
    ...
    private void GetClientes(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            objeto = clienteFacade.listaDatosCLientes();
            request.getSession().setAttribute("listaClientes", objeto);
            //request.getSession().setAttribute("listaFormaPago", formaPagoFacade.findAll());
        } catch (Exception e) {
            System.out.println("error Servlet Panel de Usuario   " + e.getMessage());
        }
